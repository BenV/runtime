...
public sealed unsafe partial class DacDbiImpl : IDacDbiInterface
{
    ...
    public int ResolveAssembly(ulong vmScope, uint tkAssemblyRef, ulong* pRetVal)
    {
        ...
        Debug.Assert((tkAssemblyRef & 0xff000000) == 0x23000000);
        ...
        *pRetVal = 0;
        ...
        {
            Contracts.ILoader loader = _target.Contracts.Loader;
            // The "scope" is the manifest module's Assembly. Get its module and look up the
            // assembly ref token in the module's ManifestModuleReferences map. The map stores
            // Module pointers; the resolved Assembly is the assembly of that module.
    ...
    {
        const uint TYPECTXT_ILNUM = unchecked((uint)-3);
        *pRetVal = 0;
        int hr = HResults.S_OK;
        try
        {
            ...
            else if (dwExactGenericArgsTokenIndex == TYPECTXT_ILNUM)
            {
    ...
    {
        // If we fail to get either a valid type handle or module, we want to send back
        // a NULL module too, so preinitialize this here.
        ...
        *pVmModule = 0;
        ...
        {
            Contracts.IRuntimeTypeSystem rts = _target.Contracts.RuntimeTypeSystem;
            // GetPrimitiveType returns a null TypeHandle if the type hasn't been loaded yet.
    ...
    public int GetGenericArgTokenIndex(ulong vmMethod, uint* pIndex)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                // (parameter index 0).
                *pIndex = rts.RequiresInstArg(md) ? TYPECTXT_ILNUM : 0u;
                hr = HResults.S_OK;
            }
        }
        catch (System.Exception ex)
